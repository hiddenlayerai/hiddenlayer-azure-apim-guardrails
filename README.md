...
`hiddenlayer-apim deploy` creates missing HiddenLayer APIM named values and policy fragments. Matching existing resources are left unchanged. Existing secret named values are skipped because Azure does not return secret values for comparison. Differing non-secret named values or policy fragments cause the command to stop with guidance.

Use `hiddenlayer-apim deploy --overwrite` when you intentionally want the CLI to update existing HiddenLayer-managed named values or replace existing HiddenLayer policy fragment XML. `apply` and `remove` preserve unrelated APIM policy rules by adding or removing HiddenLayer `<include-fragment>` references only.
...
This CLI deploys policy fragments that use HiddenLayer evaluation APIs, depending on the selected package.

### v1 Interactions (`v1-interactions`)

The v1 package uses the HiddenLayer v1 Interactions API:
...
Two v2 packages support pass-through request/response evaluation where HiddenLayer returns provider-shaped payloads:

- `v2-request-evals` calls `POST /detection/v2/request-evaluations` in inbound policy and uses `hl-runtime-action` to block before the backend call.
- `v2-response-evals` calls `POST /detection/v2/response-evaluations` in outbound policy and can replace/redact/block the backend response.
